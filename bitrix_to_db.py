...
def create_table(name_dir,table_name):
    ...
    cursor.execute(f'''
        CREATE TABLE IF NOT EXISTS {table_name} (
            номер_заявки_договора TEXT PRIMARY KEY,
            ответственный TEXT,
            вид_работ TEXT,
            фио_заказчика TEXT,
            адрес TEXT,
...
def update_db(name_dir, table_name, values):
    ...
    cursor.executemany(f'''
        INSERT OR IGNORE INTO {table_name} (
            номер_заявки_договора, ответственный, вид_работ, фио_заказчика, адрес, кадастровый_номер,
...
def backup_db(exist_db,backup_path):
